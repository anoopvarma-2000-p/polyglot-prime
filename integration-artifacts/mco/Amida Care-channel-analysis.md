# Amida Care Channel — Analysis

**File:** [`Amida Care.xml`](Amida Care.xml)
**Engine:** Mirth Connect / NextGen Connect–compatible ("BridgeLink") channel export, schema version `26.3.1`
**Channel name:** `Amida Care`
**Initial deploy state:** `STOPPED` (must be started manually after deploy)

## 1. Relationship to the Aetna channel

This channel is a **byte-for-byte structural clone** of [`Aetna.xml`](Aetna.xml) — same source connector, same ~50-rule per-record validator, same 15 destinations in the same order, same error-handling and reprocess logic. A full diff against `Aetna.xml` turns up only MCO-identity substitutions and export metadata:

| Aspect | Aetna | Amida Care |
|---|---|---|
| Channel `id` | `8624283c-…` | `85774252-b1b7-4df2-84bc-d2087d3c28f8` |
| Channel `name` | `Aetna` | `Amida Care` |
| `mco_code` (globalChannelMap) | `AET` | `AMI` |
| `mco_sftp_folder` | `mco-aetna` | `mco-amidacare` |
| Inbound SFTP host | `/lza-prod-esmf/internal/mco-aetna/prod/mirth_inbound` | `/lza-prod-esmf/internal/mco-amidacare/prod/mirth_inbound` |
| `exportData.metadata.pruningSettings` block | present (`pruneMetaDataDays=1`, archiving off) | absent from this export |

No validation rule, destination, filter, SQL statement, or error code differs between the two channels. Everything in [Aetna-channel-analysis.md](Aetna-channel-analysis.md) — the record-validation table, the destination-by-destination table, the error-handling/resiliency model, and the list of external dependencies — applies verbatim to Amida Care, substituting `mco_code = 'AMI'` and `mco_sftp_folder = 'mco-amidacare'` wherever `AET` / `mco-aetna` appear.

## 2. What this channel does (Amida Care specifics)

Polls `/lza-prod-esmf/internal/mco-amidacare/prod/mirth_inbound` for files named `AMI_MDESMF_<14-digit timestamp>.txt` (or `RS_AMI_MDESMF_<...>_<...>.txt` for resubmissions) dropped by **Amida Care** (a NY State Medicaid special-needs managed care plan), validates every pipe-delimited member record, and forwards the results:

* Valid records → `Success_<file>` → streamed to NYeC's data lake (`nyec-transfer.nyehealth.org`, `/inbound/esmf/<Month-Year>/`) and archived to `.../archive/mco-amidacare/prod/mco_success`.
* Invalid records → `Error_<file>` → copied to Amida Care's outbound error SFTP folder and archived to `.../archive/mco-amidacare/prod/mco_error`.
* Machine-readable success/rejection receipt files posted back to `${root_sftp_folder}/mco-amidacare/esmf/outbound/{success,error}`.
* Email alerts ("GRRHIO Alert: …") sent per the `emailList` config entry keyed by `mco_code = "AMI"`.
* Every file/batch/error tracked in `mco_audit.mco_records` / `mco_record_details` / `mco_record_error_logs`, keyed to the `mco_id` resolved from `mco_audit.mco where mco_code = 'AMI'`.

## 3. Architecture (identical to Aetna)

```
                         ┌───────────────────────────────────────────────────┐
                         │  SFTP inbound: /lza-prod-esmf/.../mco-amidacare/  │
        ...
        and SQL are identical, only mco_code='AMI' / mco_sftp_folder=
        'mco-amidacare' vary.
```
